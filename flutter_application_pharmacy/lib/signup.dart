...
class _SignUpState extends State<SignUp> with SingleTickerProviderStateMixin {
  ...
  Future<void> registerUser() async {
    print("Starting registerUser with email: $email, role: $selectedRole");
    try {
      ...
      print("Saved to users collection");
      ...
      if (selectedRole == 'Patient') {
        ...
        print("Saved to patients collection");
        ...
        print("Saved to caretakers collection");
        ...
        print("Saved to doctors collection");
        ...
        print("Saved to pharmacies collection");
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          duration: const Duration(milliseconds: 500), // Very short duration
        ),
      );
  ...
  Future<void> signInWithGoogle() async {
    print("Starting signInWithGoogle");
    try {
      ...
      if (user != null) {
        print("Google Auth successful, UID: ${user.uid}");
        ...
        await _firestore.collection('users').doc(docId).set({
          ...
          'role': selectedRole ?? 'Patient', // Default to Patient if not selected
          'phone': "",
          'createdAt': FieldValue.serverTimestamp(),
        }, SetOptions(merge: true));
        print("Saved to users collection");

        if (selectedRole == 'Patient' || selectedRole == null) {
          await _firestore.collection('patients').doc(docId).set({
            'docId': docId,
            'age': null,
            'gender': null,
  ...
  void handleSubmit() {
    ...
    if (emailError.isEmpty &&
        ...
        isAgreed &&
        selectedRole != null) {
      print("Validation passed, calling registerUser");
      registerUser();
    } else {
      String errorMessage = "";
      if (!isAgreed) {
